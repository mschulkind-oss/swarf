...
class TestDaemonLifecycle:
    def test_status_not_running(self, monkeypatch, tmp_path):
        import swarf.daemon.cli as dcli

        monkeypatch.setattr(dcli, "PID_FILE", tmp_path / "daemon.pid")
        runner = CliRunner()
    ...
    def test_start_creates_pid_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(dcli, "PID_FILE", pid_file)
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["daemon", "start", "--foreground"])
        assert result.exit_code == 0
